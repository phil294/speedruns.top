...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($action === 'add_category') {
		require_game_admin();
		write('insert into category (game_name, name) values (?, ?)', [$game_name, trim((string) ($_POST['name'] ?? ''))]);
	} elseif ($action === 'remove_category') {
		require_game_admin();
		write('delete from category where game_name = ? and name = ?', [$game_name, (string) ($_POST['name'] ?? '')]);
		...
		require_game_admin();
		write('insert into property (game_name, category_name, name) values (?, ?, ?)', [$game_name, (string) ($_POST['category_name'] ?? ''), trim((string) ($_POST['property_name'] ?? ''))]);
		...
		if ($role === 'admin') {
			write('insert into game_admin (game_name, user_name) values (?, ?)', [$game_name, $user_name]);
		} else {
			write('insert into game_moderator (game_name, user_name) values (?, ?)', [$game_name, $user_name]);
		}
	} elseif ($action === 'remove_moderator') {
		require_game_admin();
		write('delete from game_moderator where game_name = ? and user_name = ?', [$game_name, (string) ($_POST['user_name'] ?? '')]);
	} elseif ($action === 'verify_run') {
		write(
			'update run set verified = 1, verified_by = ? where user_name = ? and proof = ? and game_name = ?',
			[$current_user['name'], (string) ($_POST['run_user_name'] ?? ''), (string) ($_POST['run_proof'] ?? ''), $game_name],
		);
...
<fieldset>
	...
	<form method="post" action="/game/<?= e($game_name) ?>/admin">
		...
		<p><label>rules</label><br><textarea name="rules" maxlength="4000" rows="2"><?= e($category['rules']) ?></textarea> <button type="submit">save rules</button></p>
	</form>
	<div>
		properties:
...
<form method="post" action="/game/<?= e($game_name) ?>/admin">
	...
	<!-- TODO: these textareas (category rules too) are too small by default -->
	<p><label>game-wide rules</label><br><textarea name="rules" maxlength="4000" rows="2"><?= e($game['rules']) ?></textarea></p>
	<p><label>game details</label><br><textarea name="details" maxlength="4000" rows="4"><?= e($game['details']) ?></textarea></p>
